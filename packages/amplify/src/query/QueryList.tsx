import type { IUseQueryListResult as ICoreUseQueryListResult } from "@reactionable/core";

import {
	query as amplifyQuery,
	type IVariables as IAmplifyVariables,
	type IData,
	type IQueryOptions,
} from "../query/Query";
...
export type IUseQueryListResult<TData extends IData = IData> =
	ICoreUseQueryListResult<TData> & {
		next?: () => void;
		previous?: () => void;
	};

export type IQueryListOptions<
...
>(
	items: Array<TData>,
	options: IQueryListOptions<TVariables>,
): Promise<AmplifyListType<TData>> {
	...
	const shouldFetchMore =
		data.nextToken && (options.queryAll || limitNotReached);
	if (!shouldFetchMore) {
		return { ...data, items };
	}
