...
export class Paginator extends PureComponent<Props, State> {
    ...
    constructor(props: Props) {
        super(props);
        this.state = {
            pageCount: Math.round(this.props.totalCount / this.props.perPage) + (this.props.totalCount % this.props.perPage ? 1 : 0),
        };
    }
    ...
    public render(): JSX.Element {
        return <Pagination>
            ...
            <Pagination.Prev onClick={this.handlePreviousPage} disabled={this.props.currentPage === 1} />
            {this.renderPagination()}
            <Pagination.Next onClick={this.handleNextPage} disabled={this.props.currentPage === this.state.pageCount} />
            <Pagination.Last onClick={this.handleLastPage} disabled={this.props.currentPage === this.state.pageCount} />
        </Pagination>;
    }
    ...
    private renderPageLink(page: number): JSX.Element {
        return <Pagination.Item key={page} current={this.props.currentPage === page} onClick={() => this.props.onChange(page)}>{page}</Pagination.Item>;
    }
