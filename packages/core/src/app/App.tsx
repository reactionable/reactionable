import { type PropsWithChildren, type ReactElement, StrictMode } from "react";
...
import type { IRouterProviderProps } from "../router/useRouterProviderProps";
import { type IUIProviderProps, UIContextProvider } from "../ui/UI";
...
>): ReactElement {
	if (router) {
		children = (
			<RouterContextProvider {...router}>{children}</RouterContextProvider>
		);
	}

	if (identity) {
		children = (
			<IdentityContextProvider {...identity}>
				{children}
			</IdentityContextProvider>
		);
	}
