...
export function ListTableHead({ head }: IListTableHeadProps): ReactElement {
	return (
		<thead>
			<tr>
				{head?.map((item) =>
					"string" === typeof item ? <th key={item}>{item}</th> : item,
				)}
			</tr>
		</thead>
	);
...
export type IListTableComponentProps<Data extends IData = IData> =
	IListTableBodyProps<Data> & Pick<IListTableHeadProps, "head">;
...
export type IListTableProps<Data extends IData = IData> = Omit<
	IListProps<Data>,
	"children"
> &
