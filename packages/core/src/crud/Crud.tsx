import type {
	LazyExoticComponent,
	PropsWithChildren,
	ReactElement,
} from "react";

import type { IFormData, IFormValues } from "../form/Form";
import type { IFormButtonProps } from "../form/FormButton";
import type { IRouteProps } from "../router/Route";
import { useRouterContext } from "../router/useRouterContext";
import type { IModalProps } from "../ui/modal/Modal";
import type { ICreateProps } from "./create/Create";
import type { ListComponent } from "./list/List";
import type { IUseListOptions, IUseListResult } from "./list/useList";
import type { ReadComponent } from "./read/Read";
import type { IUseReadOptions, IUseReadResult } from "./read/useRead";
import type { IUpdateProps } from "./update/Update";

export interface ICrudProp<Data extends IFormData = IFormData> {
	name: string;
	listComponent?: LazyExoticComponent<ListComponent<Data>>;
	readComponent?: LazyExoticComponent<ReadComponent<Data>>;
	routes?: Array<IRouteProps>;
	privateRoute?: boolean;
}

export function Crud<Data extends IFormData = IFormData>({
	name,
	listComponent,
	readComponent,
	routes = [],
	privateRoute = true,
}: PropsWithChildren<ICrudProp<Data>>): ReactElement | null {
	const { renderRoutes, useRouteMatch } = useRouterContext();
	const match = useRouteMatch();
	if (!match) {
		return null;
	}

	const idParam = `:${name.charAt(0).toLowerCase() + name.slice(1)}Id`;

	// Add CRUD path prefix to given routes
	routes = routes.map((route) => {
		route.path = `${match.path}/${idParam}/${route.path}`;
		return route;
	});

	if (listComponent) {
		routes.push({
			index: true,
			path: match.path,
			component: listComponent,
			privateRoute: privateRoute,
		});
	}

	if (readComponent) {
		routes.push({
			index: true,
			path: `${match.path}/${idParam}`,
			component: readComponent,
			privateRoute: privateRoute,
		});
	}

	return renderRoutes(routes);
}

export type IId = string;

export type ICrudConfig<
	Values extends IFormValues = IFormValues,
	Data extends IFormData = IFormData,
	Id extends IId = IId,
	FormButtonProps extends IFormButtonProps = IFormButtonProps,
	ModalProps extends IModalProps = IModalProps,
> = {
	onCreate: ICreateProps<
		Values,
		Data,
		FormButtonProps,
		ModalProps
	>["form"]["onSubmit"];
	onUpdate: IUpdateProps<
		Values & { id: Id },
		Data,
		FormButtonProps,
		ModalProps
	>["form"]["onSubmit"];
	onDelete: (id: Id) => Promise<unknown>;
	useRead: (
		variables?: IUseReadOptions<Data>["variables"],
	) => IUseReadResult<Data>;
	useList: (
		variables?: IUseListOptions<Data>["variables"],
	) => IUseListResult<Data>;
	initialValues: ICreateProps<
		Values,
		Data,
		FormButtonProps,
		ModalProps
	>["form"]["initialValues"];
	validationSchema: ICreateProps<
		Values,
		Data,
		FormButtonProps,
		ModalProps
	>["form"]["validationSchema"];
	formChildren: ICreateProps<
		Values,
		Data,
		FormButtonProps,
		ModalProps
	>["form"]["children"];
};
