...
export type IUseMutationResult<
	TData = IData,
	TVariables extends IVariables = IVariables,
> = Omit<MutationResult<TData>, "data"> & {
...
>(
	...
	type UpdateFunction = NonNullable<
		IMutationHookOptions<TData, TVariables>["update"]
	>;

	const [mutate, { error, data, ...result }] = useMutationHook<
		TData,
		TVariables
	>(gqlQuery, {
		...options,
		...(update
			? {
					update: ((
						...
						...othersArguments: unknown[]
					) => {
						const mutationResultRecord = mutationResult as {
							data?: unknown;
						} & Record<string, unknown>;
						const updateData = extractGqlData<TData>(
							mutationResultRecord.data as IData,
						) as Unmasked<TData>;
						...
						if (othersArguments.length) {
							...
							)(cache, nextResult, othersArguments[0]) as never;
						}

						return (
							update as unknown as (cache: unknown, result: unknown) => unknown
						)(cache, nextResult) as never;
					}) as UpdateFunction,
				}
			: undefined),
	});
	...
	return {
		...
		data:
			result.called && !result.loading && !error
				? extractGqlData<TData>(data)
				: undefined,
